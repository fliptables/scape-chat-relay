...
- Enforces, per room: a 25-peer cap (HTTP 503 `room_full`) and a byte-denominated egress budget (default 20 MiB burst / 10 MiB/s sustained, charged at `frameBytes × fanout`; over-budget frames are dropped and the sender is told via `{"v":1,"type":"error","code":"room_rate_limit"}`).
...
```bash
npm test    # invariant guards + miniflare-backed suite; ~5 sec
```
